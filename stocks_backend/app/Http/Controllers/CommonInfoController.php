...
class CommonInfoController extends Controller
{
    public function getCommonValueNonbank($mack)
    {
        ...
        $column_select = DB::table("temp_table")
            ...
            ->addSelect(DB::raw('bs.so_luong_co_phieu as so_luong_co_phieu'))
        ...
        $data_capital = DB::table("capital")
            ->addSelect("mack")
            ->addSelect("thoigian")
            ->addSelect(DB::Raw('von_dieu_le'))
            ->where("mack", $mack)
            ->first();
        // $g_eps = $eps_ttm < 0 ? 0 : ($eps_ttm * $eps_previos_3_year_ttm < 0 ? 100 : $this::RATE(3, 0, - ($eps_previos_3_year_ttm), $eps_ttm));
        try {
            $res[0]['so_luong_co_phieu'] =  $data_capital->von_dieu_le * 100;
        } catch (Exception $e) {
            $res[0]['so_luong_co_phieu'] = 0;
        }        
        $res[0]['von_dieu_le'] = $res[0]['so_luong_co_phieu'] / 100;
        $res[0]['von_hoa_thi_truong'] = $price * ($res[0]['so_luong_co_phieu'] / 1000000);
    ...
    public function getCommonValueStock($mack)
    {
        ...
        if (count($res) == 0)
            return $ret;
        $res = json_decode(json_encode($res), true);
        // dd(is_null($res[0]["so_luong_co_phieu"]));
        if (!$res) {
            return $ret;
        }
    ...
    public function getCommonInfo(Request $req)
    {
        ...
        $res["cong_ty_tong_quan"] = $cong_ty_tong_quan;
