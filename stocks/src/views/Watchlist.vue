          ...
          <CCardHeader>
            <CRow>
              ...
              <CCol sm="2" class="pl-2 pr-0">
                ...
                <CButton
                style="padding: 5px;margin-right: 10px;margin-left: 10px;height: 35px"
                  color="success"
                  class="btn-add-wl"
                ...
                <CButton
                 style="padding: 5px;margin-right: 10px;margin-left: 10px;height: 35px;width:63px"
                  color="danger"
                  class="btn-update-wl"
                  @click="updateWatchlist=!updateWatchlist"
                  ><CIcon name="cibAddthis" /> Sửa</CButton
                >
            ...
            >
              <template #mack="{ item }" >
                <td
                  class=" text-center"
                >
                  <div style="color:red" v-show="updateWatchlist" @click="top(item.index_row, item.index_change_top, item.id)" v-if="item.index_row !=0" >
                  
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" c class="bi bi-caret-up-fill hover_arrow" viewBox="0 0 16 16">
                  ...
                  <CLink
                   @click="showModalTradingChart(item.mack) "
                    ...
                    ><b>{{ item.mack }}</b></CLink
                  >
                  <div  style="color:red" v-show="updateWatchlist" @click="bottom(item.index_row)"  v-if="item.index_row < list_item_canslim_fourm.length -1 ">
                    
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-caret-down-fill hover_arrow" viewBox="0 0 16 16">
              ...
              <template #note="{ item }" style="">
                <td class="text-center " style="position: relative !important;  " >
                  <button class="btn btn-info" v-on:click="isHidden = !isHidden"  
                    @click="demo(item.index_row)"
                  >
                  <span>Xem</span>
                  </button>
                  <div v-if="isHidden">
                  <div :style="{height:  300 + 'px'}" >
                  <div class=" d-flex " :class="{'justify-content-between': item.is_show_note || is_edit_note,' justify-content-end': item.is_show_note == false }"  style="margin-left: 32px;
                     ...
                     <CCollapse :show="Boolean(item.is_show_note)" v-if="item.is_show_note" class="" style="width: 88vw;">
                    ...
                    <CCollapse :show="Boolean(is_edit_note)" v-if="is_edit_note" style="width: 88vw;" >
                      <label class="font-weight-bold" style="position:absolute; left:34px" for="">Ghi chú</label>
                      <CTextarea rows="5" columns="1000" v-model="item.note" style="white-space: pre;padding: 2rem !important; " class="border rounded p-3 ">
                      </CTextarea> 
                    </CCollapse>
              ...
              <template #id="{ item }">
                <td class="text-center" v-show="updateWatchlist" >
...
export default {
  ...
  data() {
    return {
      ...
      data_test:' Các thông tin xếp hạng chấm điểm danh mục KungFuStocksPro trong KungFuStocksPro được đưa ra dựa trên cơ sở phân tích chi tiết và cẩn thận theo đánh giá chủ quan của chúng tôi là hợp lý trong thời điểm đưa ra',
    };
  ...
  methods: {
      top( index_row, id ){
        // Thay vị trí index_row của list_item_canslim_fourm
        let cache_old_index_row = this.list_item_canslim_fourm[index_row].index_row;
        this.list_item_canslim_fourm[index_row].index_row = this.list_item_canslim_fourm[index_row -1].index_row;
        this.list_item_canslim_fourm[index_row -1].index_row = cache_old_index_row;

        // Thay vị trí data của list_item_canslim_fourm
        let cache_old = this.list_item_canslim_fourm[index_row];
        this.list_item_canslim_fourm[index_row] = this.list_item_canslim_fourm[index_row - 1];
        this.list_item_canslim_fourm[index_row - 1] = cache_old;
        // Cập nhật index
        let index_change_top =  this.list_item_canslim_fourm[index_row - 1].index_row;
        let id_index_change_top = this.list_item_canslim_fourm[index_row - 1].id;
        let current_id = this.list_item_canslim_fourm[index_row].id;
        console.log(index_row)
        console.log(index_change_top)
        console.log(id_index_change_top)
        console.log(current_id)
        axios
          .post("http://127.0.0.1:8000/api/watchlist-update-index", {
            // current_id: id,
            token: localStorage.getItem("api_token"),
            // my_watchlist_id: this.list_data_model_add.current_my_watchlist_id,
            // id_index_change_top:id_index_change_top,
            // index_row: index_row,
            // index_change_top:index_change_top
          })
          .then((res) => {
      ...
      bottom( index ){
        let cache_old_index_row = this.list_item_canslim_fourm[index].index_row;
        this.list_item_canslim_fourm[index].index_row = this.list_item_canslim_fourm[index +1].index_row;
        this.list_item_canslim_fourm[index +1].index_row = cache_old_index_row;

        let cache_old = this.list_item_canslim_fourm[index];

        this.list_item_canslim_fourm[index] = this.list_item_canslim_fourm[index + 1];
        this.list_item_canslim_fourm[index + 1] = cache_old;
    ...
    getAllItem() {
      ...
      axios
        ...
        .then(function(response) { 
          let addData = response.data.map((item, index) => { 
            return { ...item, index_row: index} 
          }) 
          self.list_item_canslim_fourm = addData; 
  ...
  computed: {
    ...
    itemStockSuggestionNotClosed() {
      ...
      if (
        ...
        return_data = this.list_item_canslim_fourm.map((item, index) => {
          return {
            ...item,
